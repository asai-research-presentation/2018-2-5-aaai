...
* Planning people are working on */Automated Planner/*

+ Compute the *high-level plan* for an agent through *deliberation* (↔ reflex)
+ Subsumes motion planning
+ High-dimensional, *implicit* state space search
  + (entire space does not fit in memory)
+ STRIPS-like *PDDL modelling language*, based on
  + 1st-order logic, close-world assumption
+ Soundness and completeness matters, explainable symbolic AI
...
#+begin_row-fluid
#+begin_span8
+ Slide the tiles to an empty place
...
#+begin_row-fluid
#+begin_span12
+ To symbolic systems *names do not matter* (just matters to human)
  #+begin_src lisp
...
+ *Symbols: Labels for identifiable entities (/not necessarily have meaningful names/)*

+ There are *several kinds of symbols that can appear in PDDL*

  | Types of symbols       |                              |
  |------------------------+------------------------------|
  | Propositions           | (handempty) -- no parameters |
  | Object labels          | panel7, x0, y0 ...           |
  | Predicates (relations) | (*empty* x0 y0) (*up* y0 y1) |
...
** Can we solve an 8-puzzle without human modelling?
...
#+begin_alignright
Systems that find action symbols
#+end_alignright
...
#+begin_xlarge
#+begin_center
So far, ALL existing systems require */symbolic / near-symbolic, accurate state inputs/* and/or */discrete action labels/*.
#+end_center
#+end_xlarge
...
** Learning from Observation (Argall AIJ09, Mourao UAI12)
