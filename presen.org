...
#+begin_larger
 + *This is not suitable for our task;*
   ...
   + */there are no real answers/* for how to ground symbols
     
     #+begin_alignright
     (in fact, *people do this differently
...
* Verifying the feasibility of Latplan system

#+begin_quote
Q: Do the *propositions* found by the neural network *make any logical sense?*

Is it possible to *reason over the generated propositions?*
#+end_quote
...
** AMA_1 : Trivial Action Model Acquisition
...
+ AMA_1: feed the entire transistions, produce PDDL (note: > 1M transitions in 8-puzzle)

  + _/AMA_1 is NOT generalizing/_

+ Planner: a State-of-the-Art planner Fast Downward (Helmert, 08), $A^*$ *(optimal search)*

+ Solve the given PDDL instance and decode the results
...
*Learning / generalization from small examples is necessary*
 ...
 *action symbols, preconditions, effects*
#+end_alignright
#+end_larger
...
+ Note: Existing AMA methods all requires action symbols
...
#+begin_alignright
#+begin_larger
+ *We find these action labels without human supervision*
#+end_larger
#+end_alignright

** Are Action Symbols Necessary in a Practical Sense?

#+begin_xlarge
Yes it is. Without AAE...
#+end_xlarge
...
#+end_span4
#+begin_span8
When you have a SAE with $|z|=36 \text{bit}$, you should consider the entire
$2^{36}$ states as its neighbors, then remove the invalid transitions with AD.
#+end_span8
#+end_row-fluid
#+end_container-fluid

#+begin_center
+ *AAE provides a limited number of action labels.* Thus,

  *We can enumerate the candidate successor states in constant time*.
#+end_center
...
+ Action Learning reduces to a *prediction task*
...
#+begin_row-fluid
#+begin_span7
+ There are *multiple action labels*
+ States have *varying number of successors*
...
+ 
  #+begin_center
  *NO*, you don't know *which transition belongs to which action. /Everything is mixed up./*
  #+end_center
...
** Ok, AD sounds simple, no more unfamilier term that causes headache...

+ but hey, wait, *where do you get that training data?*
+ In particular,
...
** Negative Examples are NEVER observed in real world

In a real-world scenario, can a robot actually observe *teleportation* ?

[[png:aae/teleportation]]

The system should *imagine/generate* the false/invalid action example
in order to train the binary classifier

*but we do not know what is valid NOR WHAT IS INVALID!*

** PU-Learning framework (Elkan & Noto, KDD 08')

A method to learn a *positive/negative classifier* from *positive/mixed dataset*

Mixed dataset: some are positive, some are negative

We have:

+ Positive examples: Training input: *all data are guaranteed to be valid*
+ Mixed examples: All successor candidates generated by AAE
...
+ 100 instances for each domain , for each noise type (std, gaussian noise, salt/pepper noise)
+ (benchmark A) 7-step or  (benchmark B) 14-step self-avoiding random walks from the goal state
+ 180 sec. time limit
...
G: Gaussian noise, s/p: salt/pepper noise to the planning inputs (init/goal images)
...
| L. Out   | 100 |  99 | 100 |  59 | 60 |  51 |
| Twisted  |  96 |  65 |  98 |  75 | 68 |  72 |
| Hanoi    |  37 |  44 |  39 |  15 | 18 |  17 |

* AMA_2 Experiments 2

How accurate Action Discriminators and State Discriminators are?
...
+ State AutoEncoder(SAE): *bidirectional mapping: /real-world/ ↔ /propositional state/*
+ Action AutoEncoder(AAE): *bidirectional mapping: /transition/ ↔ /action labels/*
+ Action Discriminator(AD): *binary classifier: transition → bool* with *PU-Learning*

+ Announcing: Two of the major grounding problems were solved!
  | Types of symbols      |                              |
  |-----------------------+------------------------------|
...
+ Extending Latplan to MDP, POMDP
