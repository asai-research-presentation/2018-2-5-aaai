...
* High-Level Overview of Key Contributions
...
** Standard Classification Tasks (Supervised)
...
#+begin_larger
 + *This is not suitable for our task;*
   + */there is no label/* provided by humans
   + */no real answer/* for symbol grounding
#+end_larger
...
+ i.e. Compression: $X \rightarrow Z \rightarrow X$
+ Minimize $|x - f(x)|$ by SGD
#+end_span6
#+begin_span6
