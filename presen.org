...
Search Agent select-expand-eval
...
** Classical Planning: */typically/* with PDDL (symbolic) model

Modern planners solves *8-puzzle optimally << 0.1sec*
...
+ *Impractical* in some environments:
...
** The current state of AI agents                                  :noexport:

#+begin_container-fluid
#+begin_row-fluid
#+begin_span7
+ function SearchAgent(init, rule, goal):
  + OPEN \leftarrow \{ init \}
  + until goal found:
    + node \leftarrow select(OPEN)
    + Children \leftarrow expand(node)
    + for each c \in Children:
      + key \leftarrow evaluate(c)
      + insert(OPEN, c, key)
#+end_span7
#+begin_span1
+ 
...
+ 
+ 
+ ← covered by Reinforcement Learning
+ (DQN (Mnih et al. 16), AlphaGo (Silver et al. 15))
#+end_span1
#+begin_span1
+ 
...
+ These are beyond the capabilities of
...
** Latent-Space Planner (*/Latplan/*)                              :noexport:
...
+ No *access to the simulator* (unlike DQN for Atari ***)
  
  → cannot ask for more examples
+ 
  #+begin_larger
  No */action symbols/* (unlike any previous work to our knowledge)
...
** Verifying the feasibility of Latplan and SAE
...
AMA_1 : a */trivial, oracular/* Action Model Acquisition (AMA) method *w/o generalization*.
...
+ 2. For each transition, perform the following conversion explicitly:
  ...
  #+end_src
+ Planning with Fast Downward, A* (optimal solution) *citation*
...
*** AMA_1 Experiments                                              :noexport:
 ...
 #+begin_quote
 #+begin_center
 Show that the SAE (neural network) produce */sound propositions/*
 #+end_center
 #+end_quote

 SAE: *trained with a subset of images*
...
*AMA_1 is impractical*. Cannot obtain *all necessary valid transitions* in the real world
...
** Implementing $(a \rightarrow t) | s$                            :noexport:
...
** Collecting a negative dataset is */fundamentally/* impossible in real world
...
#+begin_alignright
#+begin_larger
+ *Invalid transitions /never happen/ → /never observed/, /no data/ available in the real world.*
#+end_larger
#+end_alignright
...
#+end_span6
#+begin_span6
+ *Action Discriminators achieve the reasonable accuracy*
...
+ *State AutoEncoder(SAE)* : */real-world states/ ↔ /propositional states/*
  ...
  *Action Discriminator(AD)* : */transition/ → /bool/* with *PU-Learning*
  ...
  | Types of symbols      | Latplan handles                    |
  |-----------------------+------------------------------------|
  | Propositional symbols | *Yes*                              |
  | Action symbols        | *Yes*                              |
  | Object symbols        | _/No/_ Computer Vision techniques? |
  | Predicate symbols     | _/No/_ ???                         |
