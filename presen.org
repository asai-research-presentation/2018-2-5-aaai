...
** The generic AI agent architechture
...
** Classical Planning: */typically/* with a PDDL (symbolic) model
...
*** Task: Solving */ANY/* Imaged-Based tasks
...
*Randomly sampled, noisy image transitions*
...
#+begin_center
(*/We skip other domains, noisy results/*)
#+end_center
#+end_larger
...
#+begin_larger
*/✗/* *AMA_1 is impractical*.
#+end_larger

AMA_1 requires *all valid transitions* in the real world

#+begin_larger
#+begin_alignright
+ → SAE + *AMA_2*, a novel neural architecture

  　which learns from small examples
#+end_alignright
#+end_larger
...
#+end_span6
#+begin_span6
*Action Discriminators achieved the reasonable accuracy*
...
+ *Latplan:* *The /first system/* which completely automatically
  generates a classical, symbolic planning model *from scratch (no human symbols)*
...
+ When this happens, */Latplan/* allows Dialogue Systems to perform:
  
  *1000 steps of natural language reasoning with /logic/, not by reflex*

  Solved by *rigorously tested State-of-the-Art Classical Planner*
...
Action Symbols (*move, interact*): modes of transitions are already segmented by human
