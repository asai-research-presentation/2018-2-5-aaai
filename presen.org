...
#+begin_alignright
+ *Impractical* in *unknown environments* when */no human/ is available*
  + 
    #+begin_larger
...
*** Why */action symbols/* are necessary?                          :noexport:
...
*** Training Input lacks *action symbols*                          :noexport:

Should identify *the number of schemas* from *what is un/affected*

[[png:ama/action-symbol]]

*** Precondition/Effect learning is not trivial                    :noexport:
...
+ *Latplan:* *The /first system/ in history* which completely automatically
  generates a symbolic planning model from scratch
...
Extracting first-order rules (predicates)
