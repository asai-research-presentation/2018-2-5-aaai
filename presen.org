...
*Search states* represented as *1st-order logic formula*, described in *PDDL language*.
...
** Can a symbolic planner solve an */Image-based/* 8-puzzle w/o human encoding?

#+begin_center
+ 3x3 Sliding Tile Puzzle: maximum 31 steps *optimal* plan

# + 4x4、5x5 : infeasible under blind search (memory exhaust)

+ *PDDL model* can be solved *optimally* *under 1sec*

+ However, *Can the same be said about /Image-based/ 8-puzzle?*
#+end_center
...
#+begin_larger
 + *This is not suitable for our task;*
   ...
   + */There are no real answers/* for how to ground propositional symbols
     ...
     cf. How many colors in a rainbow?)
     #+end_alignright
...
+ SAE with $|z|=36 \text{bit}$
  ...
  → There are *$2^{36}$ potential successors.*
...
+ → also makes *Precondition Learning harder*
