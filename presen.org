...
#+begin_alignright
+ *Impractical* in *unknown environments* where */no human/ is available*
...
(Logo embodying the lambda calculus inside a neural architecture)
...
#+begin_center
#+begin_larger
Notice that *latplan has no idea that this is an 8-puzzle*; They are entirely different from the system's point of view (state encoding is also different)
#+end_larger
#+end_center
...
*** Results with photographic, unseparated tiles (Spider 8-puzzle) :noexport:

[[png:results/spider-plan-new]]

#+begin_center
#+begin_larger
Notice that *latplan has no idea that this is an 8-puzzle*; MNIST, Mandrill, Spider are entirely different from the system's point of view (state encoding is also different)
#+end_larger
#+end_center
...
Does not assume "objects" are static (does not disappear)
...
#+begin_center
+ We have *valid transition dataset (observed data).*
...
** Datasets for Invalid transitions are */Fundamentally/* Unavailable
...
#+begin_alignright
#+begin_larger
+ *Invalid transitions /never occur/, /never observed/, /no data/ in the real world.*
#+end_larger
#+end_alignright
...
#+end_span6
#+begin_span6
+ Action Discriminators accuracy
  
  easure the type-1 / type-2 error in %
...
+ *Latplan:* *The /first system/ in history* which completely automatically
  generates a classical, symbolic planning model from scratch, w/o cheating
...
*Different SAE → reasons about different types of raw input*
